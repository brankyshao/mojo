    ...
    extends AbstractScmSettingsSetupMojoTest
{
        ...
        throws Exception
    {
        ...
        assertEquals( true, settings.isUseAuthCache() );
        assertEquals( true, settings.isTrustServerCert() );
    }
    ...
    @Override
    protected void validateAsNonUTF8EncodedTemplate()
        throws Exception
    {
        Settings settings = SvnUtil.readSettings();
        assertEquals( "configDirectory_åäöéèü", settings.getConfigDirectory() );
        assertEquals( "cygwinMountPath_åäöéèü", settings.getCygwinMountPath() );
    }
        ...
        throws Exception
    {
        ...
        assertEquals( false, settings.isUseAuthCache() );
        assertEquals( false, settings.isTrustServerCert() );
    }
}
