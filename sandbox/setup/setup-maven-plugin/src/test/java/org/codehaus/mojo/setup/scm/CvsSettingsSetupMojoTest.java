    ...
    extends AbstractScmSettingsSetupMojoTest
{
    ...
    {
        Settings settings = CvsUtil.readSettings();
        assertEquals( "changeLogCommandDateFormat_åäöéèü", settings.getChangeLogCommandDateFormat() );
        assertEquals( "temporaryFilesDirectory_åäöéèü", settings.getTemporaryFilesDirectory() );
        assertEquals( "cvsVariables_åäöéèü", settings.getCvsVariables().get( "key" ) );
    }
